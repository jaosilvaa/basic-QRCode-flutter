...
class LocalStorageService extends ChangeNotifier {
  ...
  Future<void> updateQrCode(ScanResult updateResult) async{
    final prefs = await _prefs;
    List<ScanResult> currentList = await  getSavedQrCodes();

    final index = currentList.indexWhere((item) => item.id == updateResult.id);
    if(index !=-1){
      currentList[index] = updateResult;

      final List<String> jsonStringList = currentList
            .map((res) => jsonEncode(res.toJson()))
            .toList();
      await prefs.setStringList(_savedQrCodesKey, jsonStringList);
      notifyListeners();
